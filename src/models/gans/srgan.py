...
from models.networks.modifer import CModifier, ResNetModifier_binary
...
from models.networks.common import TResidualBlock
...
class SRGAN(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_training:
            self.model_names = ['F' + str(self.gan_class), 'D' + str(self.gan_class), 'B' + str(self.gan_class)]
            self.netD = define_D(opt).to(self.device)
            self.netD.apply(weights_init)
            # self.netD = load_pretrained_net(define_D(opt), opt.trained_netD_path, self.device)
            self.netF[self.gan_class] = ResNetModifier_binary(self.u_dim, self.z_dim).to(self.device)
            self.netF[self.gan_class].apply(weights_init_zeros)
            param_G = list(self.netF[self.gan_class].parameters())
            self.optimizer_G = optim.Adam(param_G, lr=opt.g_lr, betas=(opt.beta1, opt.beta2)) # Optimize over "theta"
            ...
            def lr_lambda(iteration):
                ...
                return lr
            self.lr_schedulers = (optim.lr_scheduler.LambdaLR(self.optimizer_G, lr_lambda=lr_lambda), optim.lr_scheduler.LambdaLR(self.optimizer_D, lr_lambda=lr_lambda))
            ...
            for i in range(self.nclasses):
                self.netF[i] = CModifier(self.u_dim, self.z_dim).to(self.device)
    ...
    def forward(self, u, c):
        if c is None:
            c = self.gan_class
        labels = torch.ones(u.shape[0]).to(torch.long) * c
        z = self.netF[c](u.to(self.device), labels.to(self.device))
        return self.pre_netG(z)

    def backward_D(self, reals, fakes):
        # real samples 
        labels = torch.ones(reals.shape[0]).to(torch.long) * self.gan_class
        dis_out_real, _ = self.netD(reals, labels.to(self.device))
        dis_out_fake, _ = self.netD(fakes, labels.to(self.device))
        self.loss_D = loss_hinge_dis(dis_out_real, dis_out_fake)

    def backward_G(self, fakes):
        labels = torch.ones(fakes.shape[0]).to(torch.long) * self.gan_class
        gen_out_fake, _ = self.netD(fakes, labels.to(self.device))
    ...
    def save_networks(self, epoch=0):
        ...
        # save_(self.netB, 'B' + str(self.gan_class))
    ...
    def load_networks(self, epoch=-1):
        ...
        if self.is_training:
            ...
            # load_(self.netB, 'B' + str(self.gan_class))
        else:
            load_(self.netF[self.gan_class], 'F' + str(self.gan_class))
            # load_(self.netB, 'B' + str(self.gan_class))
            # load all F
