...
class Generator(nn.Module):
    ...
    def forward(self, z, label):
        y = label.clamp(None, self.nlabels - 1)
...
class Discriminator(nn.Module):
    def __init__(self,
        ...
        #     nn.utils.spectral_norm(nn.Embedding(num_classes, self.embed_dim)),
        #     nn.Linear(self.embed_dim, num_classes)
        #     )
    ...
    #     x = self.embed(inputs)
    #     x_proj = torch.sum(torch.mul(self.linear_y[1](labels), x), dim=1, keepdim=True)
    ...
    def forward(self, x, label=None, get_features=False):
        input = self.stack(x)
        out = self.conv1(x)
        out = self.conv2(out)
        out = self.conv3(out)
