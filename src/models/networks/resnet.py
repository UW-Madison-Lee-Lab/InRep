...
class GenBlock(nn.Module):
    ...
    def forward(self, x, label):
        ...
        # with profiler.record_function("INTERPOLATE"):
...
class Generator(nn.Module):
    """Generator."""
    def __init__(self, z_dim, shared_dim, img_size, img_channel, g_conv_dim, g_spectral_norm, attention, attention_after_nth_gen_block, activation_fn,
        ...
        if g_spectral_norm:
            self.conv2d5 = snconv2d(in_channels=self.out_dims[-1], out_channels=img_channel, kernel_size=3, stride=1, padding=1)
        else:
            self.conv2d5 = conv2d(in_channels=self.out_dims[-1], out_channels=img_channel, kernel_size=3, stride=1, padding=1)
...
class Discriminator(nn.Module):
    """Discriminator."""
    def __init__(self, img_size, img_channel, d_conv_dim, d_spectral_norm, attention, attention_after_nth_dis_block, activation_fn, conditional_strategy,
                 hypersphere_dim, num_classes, nonlinear_embed, normalize_embed, initialize, D_depth, mixed_precision):
        super(Discriminator, self).__init__()
        d_in_dims_collection = {"32": [img_channel] + [d_conv_dim*2, d_conv_dim*2, d_conv_dim*2],
                                "64": [img_channel] +[d_conv_dim, d_conv_dim*2, d_conv_dim*4, d_conv_dim*8],
                                "128": [img_channel] +[d_conv_dim, d_conv_dim*2, d_conv_dim*4, d_conv_dim*8, d_conv_dim*16],
                                "256": [img_channel] +[d_conv_dim, d_conv_dim*2, d_conv_dim*4, d_conv_dim*8, d_conv_dim*8, d_conv_dim*16],
                                "512": [img_channel] +[d_conv_dim, d_conv_dim, d_conv_dim*2, d_conv_dim*4, d_conv_dim*8, d_conv_dim*8, d_conv_dim*16]}
        ...
        self.embed_dim = self.out_dims[-1]
        self.authen = nn.Linear(self.embed_dim, num_classes)
        # nn.Linear(self.embed_dim, self.embed_dim//2),
            # nn.BatchNorm1d(self.embed_dim//2),
            # nn.ReLU(),
        self.proj = nn.utils.spectral_norm(nn.Embedding(num_classes, self.embed_dim))
        self.aux = nn.Linear(self.embed_dim, num_classes)
        self.linear_y = nn.ModuleList([self.authen, self.proj, self.aux])
    ...
    def projection(self, inputs, labels):
        ...
        x_proj = torch.sum(torch.mul(self.proj(labels), x), dim=1, keepdim=True)
        x_aux = self.aux(x)
