...
def weights_init_zeros(m):
    if isinstance(m, nn.BatchNorm2d) or isinstance(m, CondBatchNorm2d) or isinstance(m, nn.BatchNorm1d) or isinstance(m, CondBatchNorm1d):
    ...
    if isinstance(m, CustomConv2d):
        if m.conv.weight is not None:
            init.constant_(m.conv.weight.data, 0.0)
...
class TResidualBlock(nn.Module):
    def __init__(self,
                 ...
                 kernel_size,
                 spectral_norm=False):
        super(TResidualBlock, self).__init__()
...
class DepthToSpace(nn.Module):
    ...
    def forward(self, x):
        ...
        output = torch.stack(output_list, 0).transpose(0, 1).contiguous().permute(
...
class UncondResidualBlock(nn.Module):
    def __init__(self,
                 ...
                 resample=None,
                 spectral_norm=False):
        super(UncondResidualBlock, self).__init__()
        if in_channels != out_channels or resample is not None:
            self.learnable_shortcut = True
        else:
            self.learnable_shortcut = False
