...
class Glow(nn.Module):
    ...
    def prior(self, size, y_onehot=None):
        if size is not None:
            h = self.prior_h.repeat(size, 1, 1, 1)
        else:
            # Hardcoded a batch size of 32 here
            h = self.prior_h.repeat(32, 1, 1, 1)
    ...
    # def forward(self, x=None, y_onehot=None, z=None, temperature=None, reverse=True): # change from False to True for reverse
    #     if reverse:
    #         return self.reverse_flow(z, y_onehot, temperature)
    #     else:
    #         return self.normal_flow(x, y_onehot)

    def forward(self, x=None, y_onehot=None, z=None, temperature=1, reverse=True, size=32): # change from False to True for reverse
        if reverse:
            return self.reverse_flow(z, y_onehot, temperature, size)
        else:
            return self.normal_flow(x, y_onehot)
    ...
    def reverse_flow(self, z, y_onehot, temperature, size=None):
        with torch.no_grad():
            if z is None:
                mean, logs = self.prior(size, y_onehot)
