...
def batch_pairwise_distances(U, V):
    ...
    N = U.shape[0]
    M = V.shape[0]
    norm_u = torch.sum(U**2, dim=1, keepdim=True).expand(N, M)
    norm_v = torch.sum(V**2, dim=1, keepdim=True).expand(M, N).transpose(0, 1)
    
    # Pairwise squared Euclidean distances.
    uv = torch.matmul(U, V.transpose(0, 1))
    D = F.relu(norm_u - 2*uv + norm_v)
...
if __name__ == '__main__':
    N = 10
    D = 16
    real_features = torch.randn(10, D)
    generated_features = torch.randn(12, D)
    state = knn_precision_recall_features(real_features, generated_features)
